...
- `delivery.delivery_time` — поле з Poster Web API / частини runtime payload
...
Backend трактує цей час як локальний час Poster-точки. Для поточного продового сценарію всі точки працюють у часовому поясі `Europe/Kiev`, тому перед відправкою в Shipday час конвертується в UTC, як того вимагає Shipday API.

Тобто в інтерфейсі Shipday має відображатися той самий локальний час доставки, який касир бачить у Poster, але в API Shipday ми шлемо вже UTC-еквівалент.

Додатковий fallback:

- якщо POS bundle не побачив scheduled time у runtime payload,
- backend викликає `dash.getTransaction` з `include_delivery=true&timezone=client`
- і бере час із `delivery.delivery_time`

Це потрібно саме для preorder / scheduled delivery, де POS runtime іноді не віддає час доставки у верхньому payload, хоча Web API його містить.
